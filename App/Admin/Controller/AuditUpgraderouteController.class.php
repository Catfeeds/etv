...
// | Author: Blues
// +-------------------------------------------------------------------------
// | Description: 审核管理--路由固件审核
// +-------------------------------------------------------------------------
namespace Admin\Controller;
use Admin\Controller\ComController;
class AuditUpgraderouteController extends ComController {
    ...
    public function changeAuditStatus($audit_status){
        ...
        if($result === false){
            ...
            if($audit_status == 1){
               addAuditlog("路由固件审核未通过,数据ID为：".$idsstr,$resouce_type=5,$type=1);
            }elseif($audit_status == 2){
               addAuditlog("路由固件审核通过,数据ID为：".$idsstr,$resouce_type=5,$type=1);
            }
            $this->success("修改状态成功");
        }
    }
