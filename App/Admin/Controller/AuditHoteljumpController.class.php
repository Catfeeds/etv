...
use Vendor\Tree;
class AuditHoteljumpController extends ComController {
    ...
    public function changeAuditStatus($audit_status){
        ...
        if($result === false){
            ...
            if($audit_status == 1){
               addAuditlog("跳转视频审核未通过,数据ID为：".$idsstr,$resouce_type=1,$type=1);
            }elseif($audit_status == 2){
               addAuditlog("跳转视频审核通过,数据ID为：".$idsstr,$resouce_type=1,$type=1);
            }
            $this->success("修改状态成功");
        }
    }
