...
// | Author: Blues
// +-------------------------------------------------------------------------
// | Description: 发布管理--路由固件发布
// +-------------------------------------------------------------------------
namespace Admin\Controller;
use Admin\Controller\ComController;
class PushUpgraderouteController extends ComController {
    public function index(){
        $model = M("UpgradeRoute");
    ...
    public function changeAuditStatus($audit_status){
        ...
        if($result === false){
            ...
            if($audit_status == 3){
               addAuditlog("路由固件发布未通过,数据ID为：".$idsstr,$resouce_type=15,$type=2);
            }elseif($audit_status == 4){
               addAuditlog("路由固件发布通过,数据ID为：".$idsstr,$resouce_type=15,$type=2);
            }
            $this->success("修改状态成功");
        }
    }
