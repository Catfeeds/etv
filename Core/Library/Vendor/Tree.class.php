...
class Tree {
	...
	* @return array
	*/
	public function get_all_childids($myid){
		$child = array();
		$ids = array();
	...
	*/
	public function get_tree($myid, $str, $sid = 0, $adds = '', $str_group = ''){
		$number=1;
		$child = $this->get_child($myid);//得到pid为$myid的集合 有前提查询条件（子集不能作为上级）
		if(is_array($child)){ 
		    $total = count($child);
			foreach($child as $id=>$value){ //只有有子集时 才跳出循环
				$j=$k=''; 
				if($number==$total){ //判断二级循环 确定最后一个的名称前标
					$j .= $this->icon[2];
				}else{
					$j .= $this->icon[1];
					$k = $adds ? $this->icon[0] : '';
				}
				$spacer = $adds ? $adds.$j : '';//名称前标
				$selected = $value['id']==$sid ? 'selected' : '';//是否被选中
				@extract($value);//将变量导入到当前的符号表		
